...
<template>
  <div class="fullScreen" id="fullScreen">
        <img class="rotating" :src="require('@/assets/svg/spaceman.svg')">
    <div class="pagenotfound-text">
      <router-link to="/">
      <h1>Page lost in space</h1>
    </router-link>
    </div>
    <canvas id="canvas2d" width="1589" height="764"></canvas>
  </div>
</template>
<script>
  export default {
    ...
    mounted: function() {
      ...
      StarField.prototype._updateStarField = function() {
        ...
        for (i = 0; i < this.numStars; i++) {
          ...
          if ((Math.abs(star.x) > this.width / 2) ||
            ...
            //);
            randomLoc = BigBang.getRandomPosition(-this.width / 10, -this.height / 10,
      ...
      StarField.prototype._adjustCanvasSize = function(width, height) {
        ...
        this.height = this.canvasElem.height = 1000;
        // 这地方之前的逻辑是实时计算height, 现在设置为固定size
