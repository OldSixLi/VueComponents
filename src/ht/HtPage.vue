  ...
  <p style="margin:15px 0;width:100%;" class="text-right ht-page">
    <nav aria-label="Page navigation" :data-option="lists" class="page-nav">
      <ul class="pagination">
        ...
        <!-- 额外添加首页和省略号 -->
        <li @click="currentPage=1;pageClick()" v-if="(pageList[0]-1)>0">
          <a href="javascript:;" title="第1页">1</a>
        </li>
        <li @click="currentPage=2;pageClick()" v-if="(pageList[0]-1)==2">
          <a href="javascript:;" title="第2页">2</a>
        </li>
        <li v-if="(pageList[0]-1)>2">
          <a href="javascript:;">...</a>
        </li>
        <!-- END:额外添加首页和省略号 -->
  ...
  computed: {
    ...
    pageList: function() {
      ...
      if (_self.totalPage <= _self.showItem) {
        ...
        if (_self.currentPage > Math.ceil(_self.showItem / 2)) {
          if (
            _self.totalPage - _self.currentPage >=
            Math.floor(_self.showItem / 2)
          ) {
            for (
              //NOTE 此处一个为floor,一个为ceil 是为了控制showItem 奇偶数的问题
              let index = _self.currentPage - Math.floor(_self.showItem / 2);
              index < _self.currentPage + Math.ceil(_self.showItem / 2);
              index++
            ) {
              pageList.push(index);
            }
            ...
            for (
              let index = _self.totalPage - _self.showItem + 1;
              index <= _self.totalPage;
              index++
            ) {
              pageList.push(index);
            }
    ...
    toPage: function(index) {
      ...
      if (!isNaN(index)) {
        if (index - 0 != this.currentPage) {
          this.currentPage =
            index - 0 > this.totalPage ? this.totalPage : index - 0;
          this.PageNum =
            index - 0 > this.totalPage ? this.totalPage : index - 0;
