    ...
    export default {
        ...
        methods: {
            ...
            getDomValue(name) {
                ...
                switch (type) {
                    case "radio":
                        return $(_self.$el).find("[name='" + name + "']:checked").val();
                        break;
                    case "checkbox":
                        var item = [];
                    ...
                    if (!isNaN(_self.min)) {
                        if (_self.inputType != 'checkbox') {
                            ...
                            if (value == [] || value.length < Number(_self.min)) {
                                _self.validateError = true;
                                _self.errorMessage = _self.minMessage ? _self.minMessage : "请至少选择" + _self.min + "项";
                                _self.errorMessageList.push(_self.errorMessage);
                            }
                        }
                    } else {
                    ...
                    if (!isNaN(_self.max)) {
                        if (_self.inputType != 'checkbox') {
                            ...
                            if (value == [] || value.length > Number(_self.max)) {
                                _self.validateError = true;
                                _self.errorMessage = _self.maxMessage ? _self.maxMessage : "最多选择" + _self.max + "项";
                                _self.errorMessageList.push(_self.errorMessage);
                            }
                        }
                    } else {
